...
public class ThreadCercatore implements Runnable {
    ...
    public ThreadCercatore(Nodo node, int ragg) {
        this.node = node;
        this.ragg = ragg;
    }
    ...
    public Set<NodoGooglePlaces> prendiSetDaNodo(Nodo nodo, int raggio, String token) {
        ...
        InterpretatoreGooglePlaces.lock.lock();
        try
        {
            InterpretatoreGooglePlaces.pubblicoSet.addAll(set);
        }
        finally {
            InterpretatoreGooglePlaces.lock.unlock();
        }
        return set;
    }
}
